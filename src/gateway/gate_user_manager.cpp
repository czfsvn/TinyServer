...
GateUserPtr GateUserManager::addUser(uint32_t user_id)
{
    ...
    if (it != users_.end())
    {
        LOG_WARN("User {} already exists", user_id);
        return it->second;
    }

    auto user = std::make_shared<GateUser>(user_id);
    users_[user_id] = user;
    LOG_INFO("Added user: {}", user_id);
    return user;
}
...
size_t GateUserManager::getOnlineUserCount() const
{
    ...
    for (const auto& pair : users_)
    {
        if (pair.second->getState() == UserState::Online)
        {
            count++;
        }
    }
...
size_t GateUserManager::getAuthenticatedUserCount() const
{
    ...
    for (const auto& pair : users_)
    {
        if (pair.second->getState() >= UserState::Authenticated)
        {
            count++;
        }
    }
...
size_t GateUserManager::cleanupTimeoutUsers(std::chrono::seconds timeout)
{
    ...
    while (it != users_.end())
    {
        ...
        {
            LOG_WARN("Cleaning up timeout user: {}", it->first);
            it = users_.erase(it);
            removed++;
        }
        else
